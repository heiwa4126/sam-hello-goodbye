...
```bash
hello-goodbye$ sam build --use-container
```
...
```bash
hello-goodbye$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
hello-goodbye$ sam local start-api
hello-goodbye$ curl http://localhost:3000/
```
...
```bash
hello-goodbye$ sam logs -n HelloWorldFunction --stack-name hello-goodbye --tail
```
...
```bash
hello-goodbye$ pip install -r tests/requirements.txt --user
# unit test
hello-goodbye$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
hello-goodbye$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name hello-goodbye
```
